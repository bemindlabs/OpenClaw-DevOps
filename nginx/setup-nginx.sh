...
echo ""
echo "เนื่องจากคุณมี SSL certificate อยู่แล้วบน router (YOUR_PUBLIC_IP)"
echo "ให้คัดลอกมาใช้บนเครื่อง Mac นี้:"
echo ""
echo -e "${GREEN}# SSH เข้า router และหา certificate${NC}"
echo "ssh root@YOUR_PUBLIC_IP"
echo "find /etc -name '*your-domain.com*' -o -name '*letsencrypt*'"
echo ""
echo -e "${GREEN}# คัดลอกมาที่เครื่อง Mac${NC}"
echo "scp root@YOUR_PUBLIC_IP:/etc/letsencrypt/live/your-domain.com/fullchain.pem ./nginx/ssl/cert.pem"
echo "scp root@YOUR_PUBLIC_IP:/etc/letsencrypt/live/your-domain.com/privkey.pem ./nginx/ssl/key.pem"
echo ""
...
echo -e "${BLUE}════════════════════════════════════════════${NC}"
echo ""
echo "1️⃣  Uninstall nginx บน router (YOUR_PUBLIC_IP):"
echo ""
echo "   SSH เข้า router:"
echo -e "   ${GREEN}ssh root@YOUR_PUBLIC_IP${NC}"
echo ""
...
echo "   • Port 80 (HTTP)  → YOUR_PRIVATE_IP:80"
echo "   • Port 443 (HTTPS) → YOUR_PRIVATE_IP:443"
echo ""
echo "   วิธีตั้งค่า:"
...
echo -e "   ${GREEN}curl https://your-domain.com/health${NC}"
echo ""
echo -e "${BLUE}════════════════════════════════════════════${NC}"
echo ""
